...
class DM:
    def __init__(self, w, n):
        ...
        self.w = w
        self.n = n
        (self.all_types, self.all_weights, self.all_logsizes, self.all_log2cumsizes) = (
            self.gettypesizes(w, n)
        )
    ...
    def gettypesizes(w, n):
        ...
        types = np.concatenate(
            (types, n - np.sum(types, 1, keepdims=True, dtype=np.uint16)), 1
        )
    ...
    def set_mcdm_rate(self, k_bits):
        ...
        d_size = np.exp(self.all_log2cumsizes[max_idx] * np.log(2)) - np.exp(
            k_bits * np.log(2)
        )
    ...
    def set_ccdm_rate(self, k_bits):
        ...
        if len(indices) == 0:
            assert (
                False
            ), f"k_bits={k_bits} not supported, largest value is {np.max(self.all_logsizes) / np.log(2)}"
        max_idx = indices[0]
